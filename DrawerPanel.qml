// Adapted from Omarchy Ui/KeyboardPanel.qml (2026-09-08), MIT; see LICENSE.
// Retains focus priming, bar handoff, and dismissal on other monitors.
import QtQuick
...
PanelWindow {
  ...
  property int padding: Style.space(20)
  ...
  // Keep the card and shadow behind the bar's inner edge on every bar side.
  Item {
    id: drawerViewport
    x: root.barPos === "left" ? root.barW : 0
    y: root.barPos === "top" ? root.barH : 0
    width: Math.max(0, root.screenW - x - (root.barPos === "right" ? root.barW : 0))
    height: Math.max(0, root.screenH - y - (root.barPos === "bottom" ? root.barH : 0))
    clip: true
    ...
    RectangularShadow {
      ...
      radius: card.radius
      blur: Style.space(32)
      offset: Qt.vector2d(0, Style.space(12))
    ...
    BorderSurface {
      id: card
      readonly property real openX: root.cardOrigin.x - drawerViewport.x
      readonly property real openY: root.cardOrigin.y - drawerViewport.y
      readonly property real closedX: root.barPos === "left" ? -width - Style.space(40)
        : root.barPos === "right" ? drawerViewport.width + Style.space(40) : openX
      readonly property real closedY: root.barPos === "bottom" ? drawerViewport.height + Style.space(40)
        : root.barPos === "top" ? -height - Style.space(40) : openY
      x: closedX + root.reveal * (openX - closedX)
      y: closedY + root.reveal * (openY - closedY)
      width: root.contentWidth
      height: root.contentHeight
      color: Qt.rgba(Color.popups.background.r, Color.popups.background.g, Color.popups.background.b, 0.97)
